...
void setup() {
  ...
  // Create an array of 100 particles //
  points = new Particle[NUM_PARTICLES];
...
void refresh() {
  for (int i=0; i<NUM_PARTICLES; i++) {
    points[i].place(random(width),random(height));
  }
}
...
class Particle {
  ...
  void update() {
    xv = cos(noise(x,y)*TWO_PI);
    yv = sin(noise(x,y)*TWO_PI);
